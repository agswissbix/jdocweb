...
class Sys_model extends CI_Model {
    ...
    function get_customsearch_query($search_tableid,$tables,$custom_fields)
    {
        ...
        if($search_tableid=='contratti')
        {
           $colore_oresupplementari=$custom_fields['colore_oresupplementari'];
           if($colore_oresupplementari=='Rosso')
           {
               $customsearch_query=$customsearch_query." AND (oresupplementari2 like '%-%')"; 
           }  
           if($colore_oresupplementari=='Verde')
           {
               $customsearch_query=$customsearch_query." AND (oresupplementari2 not like '%-%' and oresupplementari2 is not null)"; 
           }   
	...
	GROUP BY recordiddipendenti_,nome,cognome
            ...
            function custom3p_genera_listaclienti()
            {
                ...
                $sql="
                    ...
                    FROM
                    (
                    SELECT recordidazienda_,ultimadata,T2.recordid_,T2.ragionesociale,T2.statolistacliente
                    FROM 
                    (
                            SELECT recordidazienda_,MAX(DATA) as ultimadata
                            FROM user_rapportidilavoro AS r
                            WHERE  r.data>='2021-07-22'
                            GROUP BY recordidazienda_
                            ) AS T1
                            LEFT JOIN
                            (
                            SELECT recordid_,ragionesociale,statolistacliente
                            FROM user_azienda
                            )
                            AS T2 ON T1.recordidazienda_=T2.recordid_
                     ...
                     UNION
                            ...
                            SELECT *
                            FROM user_azienda
                            WHERE statolistacliente='Cliente'
                            )
                            AS T2 ON T1.recordidazienda_=T2.recordid_
                    ) AS T3
                    WHERE recordid_ IS NOT null
                    ORDER BY T3.ragionesociale
                    ";
               ...
               // $counter=1;
                foreach ($aziende as $key => $azienda) {
                    ...
                    //echo $counter.":".date("Y-m-d H:i:s")."Elaborazione: ".$azienda['recordid_']."<br/>";
                    //$this->custom3p_genera_listaclienti_azienda($azienda['recordid_']);
                    //$counter++;
                }
                return $clienti;        
            }
            ...
            {
                    $sql="DELETE FROM user_listaclienti WHERE recordidazienda_='$recordid_azienda'";
                    $this->execute_query($sql);
                    $azienda= $this->get_record('azienda',$recordid_azienda);
                    ...
                    $today=  date('Y-m-d');
                    $ccl_recordid=$this->db_get_value("user_ccl","recordid_","nomeccl='$cclname' AND  stato = 'Verificato' AND (('$today'>=datainizio AND datafine is null) OR  ('$today'>=datainizio AND '$today'<=datafine))","ORDER BY recordid_ desc");
                    ...
                    $datalimite=date('Y-m-d',strtotime('-13 months'));
                    $sql="
                        SELECT data
                        FROM user_rapportidilavoro AS r
                        WHERE r.recordidazienda_='$recordid_azienda' AND r.data>='$datalimite'
                    ...
                    if($result!=null)
                    {
                        ...
                        $fields['num']= $this->generate_seriale('listaclienti', 'num');
                    }
                    $fields['id']= $this->generate_seriale('listaclienti', 'ID');
                    
                    $offerta_prezzi=$this->db_get("user_offerta_prezzi","*","recordidazienda_='$recordid_azienda'","ORDER BY recordid_ DESC","LIMIT 6");
                    $fields['1prezzo']=" #$ccl_rgb";
                    if(array_key_exists(0, $offerta_prezzi))
                    {
                        $fields['1prezzo']=$offerta_prezzi[0]['prezzo']."#$ccl_rgb";
                    }        
                    $fields['2prezzo']=" #$ccl_rgb";
                    if(array_key_exists(1, $offerta_prezzi))
                    {
                        $fields['2prezzo']=$offerta_prezzi[1]['prezzo']."#$ccl_rgb";
                    }   
                    $fields['3prezzo']=" #$ccl_rgb";
                    if(array_key_exists(2, $offerta_prezzi))
                    {
                        $fields['3prezzo']=$offerta_prezzi[2]['prezzo']."#$ccl_rgb";
                    }   
                    $fields['4prezzo']=" #$ccl_rgb";
                    if(array_key_exists(3, $offerta_prezzi))
                    {
                        $fields['4prezzo']=$offerta_prezzi[3]['prezzo']."#$ccl_rgb";
                    }   
                    $fields['5prezzo']=" #$ccl_rgb";
                    if(array_key_exists(4, $offerta_prezzi))
                    {
                        $fields['5prezzo']=$offerta_prezzi[4]['prezzo']."#$ccl_rgb";
                    }   
                    $fields['6prezzo']=" #$ccl_rgb";
                    if(array_key_exists(5, $offerta_prezzi))
                    {
                        $fields['6prezzo']=$offerta_prezzi[5]['prezzo']."#$ccl_rgb";
                    }   
                    
                    $fields['acquisitoda1']=$azienda['acquisitoda1'];
                    $fields['acquisitoda2']=$azienda['acquisitoda1'];
                    $fields['acquisitoda3']=$azienda['acquisitoda1'];
                    
                    $nome1=$this->db_get_value("sys_user","firstname","id='".$azienda['acquisitoda1']."'");
                    $nome2=$this->db_get_value("sys_user","firstname","id='".$azienda['acquisitoda2']."'");
                    $nome3=$this->db_get_value("sys_user","firstname","id='".$azienda['acquisitoda3']."'");
                    
                    $fields['acquisitoda']=$nome1;
                    if($this->isnotempty($nome2))
                    {
                        $fields['acquisitoda']=$fields['acquisitoda']." / ".$nome2;
                    }
                    
                    if($this->isnotempty($nome3))
                    {
                        $fields['acquisitoda']=$fields['acquisitoda']." / ".$nome3;
                    }
                    
                    
                    $fields['datacompilazionerapportini']=$azienda['datacompilazionerapportini'];
                    $fields['datalavoroal100']=$azienda['datalavoroal100'];
